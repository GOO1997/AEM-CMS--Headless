...
const serviceURL = `http://${import.meta.env.VITE_GQL_DEV_SERVER_HOSTNAME}:${
  import.meta.env.VITE_GQL_DEV_SERVER_PORT
}`;
    ...
    <section className="adventures">
      {adventures.data.adventureList.items.map((item) => {
        return (
          <div key={item._path}>
            <img
              alt={item.adventureTitle}
              src={`${serviceURL}${item.adventurePrimaryImage._path}`}
            />
            <h2>{item.adventureTitle}</h2>
            <p>{item.adventurePrice}</p>
